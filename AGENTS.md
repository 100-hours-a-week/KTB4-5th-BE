...
- **메서드** 권장 20줄, 상한 30줄. **클래스** 권장 200줄, 상한 300줄. 줄바꿈된 체이닝(`.orElseThrow`)도 줄마다 센다. **한 줄 100자.**
- **DDD는 적용하지 않는다.** 패키지는 도메인(기능)별 파일 분리 목적이며 `<domain>/presentation·application·domain·infrastructure` 구조를 따른다.
